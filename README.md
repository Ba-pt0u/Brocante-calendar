...
- Pagination spéculative : si aucun marqueur HTML, essaie automatiquement `?page=N+1`
  (utilisé par brocabrac.fr dont les URLs `/dept/slug/` n'incluent pas de lien suivant)
- Arrêt dès que la page suivante ne contient que des doublons déjà vus

**Performances :**
- Sources scrappées en **parallèle** via `asyncio.gather()`
- Événements mis en **cache mémoire** (`_state["events"]`) pour éviter les relectures JSON
...
- Déduplication par hash MD5 `titre|date|lieu` (intra-source et inter-sources)
...
│   └── test_scraper.py         ← 79 tests unitaires + intégration + @live
├── templates/
...
```bash
# Lancer tous les tests (276 tests, ~8s)
pytest
