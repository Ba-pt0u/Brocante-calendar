...
```
{Ville} — {emoji} {Type} — {Titre original} {emojis contextuels}
```

Exemple : `Saint-Arnoult — 🛍️ Brocante — Grande Brocante annuelle 🍕🎪`

La ville affichée suit cette priorité : nom de commune utilisé comme lieu (champ unique sans espace) > `addressLocality` du JSON-LD > géocodage Nominatim > extraction depuis la chaîne de lieu. Les noms de salles/rues (« Salle des fêtes », « Rue d'Arras »…) sont ignorés.

| Emoji | Type d'événement |
|---|---|
| 🛍️ | Brocante |
| 📦 | Vide-grenier |
| 👗 | Vide-Dressing |
...
|---|---|---|
| `GET` | `/` | Interface web |
| `GET` | `/feed.ics` | Flux ICS complet avec ETag (abonnement calendrier) |
| `GET` | `/feed.ics?types=brocante,vide-grenier` | Flux ICS filtré par type(s) |
| `GET` | `/api/config` | Configuration actuelle |
| `POST` | `/api/config` | Mettre à jour la config + refresh immédiat |
| `GET` | `/api/events` | Liste des événements scrappés (filtres: `?types=`, `?dept=`, `?within_days=`) |
...
- **Carte interactive** (Leaflet.js) : bouton 🗺️ affiche les événements géocodés sur une carte OpenStreetMap. Clic sur un marqueur → titre + date + lieu + lien.
- **Recherche plein texte** : champ de recherche filtrant instantanément les cartes par titre et lieu.
...
- **Abonnement filtré** : dans la section ICS, des boutons permettent de sélectionner un ou plusieurs types d'événements. L'URL d'abonnement se met à jour automatiquement (ex: `/feed.ics?types=brocante,vide-grenier`). Le nom du calendrier reflète les types choisis (ex: `🛍️ Brocante · Vide-grenier – Lyon`).
- **Ville dans les cartes** : la commune est affichée en badge gris dans le titre des cartes. La priorité est la même que pour l'ICS : `addressLocality` JSON-LD > géocodage Nominatim > extraction du lieu. Les noms de salles/rues sont filtrés.
...
## Source et stratégie de scraping

**Source unique : brocabrac.fr**

| URL de recherche |
|---|
| `https://brocabrac.fr/{dept}/{commune-slug}/` (ex : `/78/saint-arnoult-en-yvelines/`) |
...
**Filtre de distance :**
Après géocodage, les événements dont les coordonnées dépassent `rayon × 1.15 km` du centre de recherche sont éliminés côté serveur.

**Parsing :**
JSON-LD `<script type="application/ld+json">` (schéma `Event`) — rapide et fiable.

**Pagination :**
- Détection HTML via `<link rel="next">` / `<a rel="next">` / `<a class="next">`
- Pagination spéculative : si aucun marqueur HTML, essaie automatiquement `?page=N+1`
  (brocabrac.fr n'inclut pas de lien « suivant » dans ses URLs `/dept/slug/`)
- Arrêt dès que la page suivante ne contient que des doublons déjà vus
...
│   ├── scraper.py              ← scraping brocabrac.fr, dates françaises, retry
...
│   ├── test_api.py             ← 75 tests d'intégration FastAPI
│   ├── test_calendar_gen.py    ← 74 tests unitaires ICS
│   ├── test_config.py          ← 20 tests unitaires config
│   ├── test_date_parser.py     ← 28 tests unitaires parseur de dates
│   └── test_scraper.py         ← tests unitaires + intégration + @live
├── templates/
